...
Droid::Droid(Droid const& droid) : Id(droid.Id), Energy(droid.Energy), Attack(droid.Attack), Toughness(droid.Toughness), Status(new std::string(*(droid.Status))), BattleData(new DroidMemory(*(droid.BattleData)))
{
...
bool Droid::operator()(std::string const* task, size_t required)
{
  ...
  if (Energy <= 10)
    ...
    {
      Energy -= 10;
      if (BattleData->getExp() >= required)
	{
	  BattleData->setExp(BattleData->getExp() + required/2);
	  this->setStatus(new std::string(stat[1]));
	  return (true);
	}
      else
	{
	  BattleData->setExp(BattleData->getExp() + required);
